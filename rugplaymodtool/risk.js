...
(() => {
  ...
  // Helper: calculate top 3 holder risk (smaller effect)
  ...
  function insertRiskCard(ownerName, ownerPercent, topHolders) {
    ...
    const reasons = [`Owner owns ${ownerPercent}% → ${riskPoints > 0 ? "+" : ""}${riskPoints} pts`];

    // Remove owner from top holders if present
    ...
    if (ownerPercent - maxOther > 50) {
      ...
      if (riskPoints > 0) riskPoints = getOwnerRisk(ownerPercent); // only owner counts
    }
    ...
    card.innerHTML = `
      ...
      <div style="font-size:13px;margin-bottom:6px;"><b>Risk Points:</b> ${riskPoints}<br><b>Risk Level:</b> ${riskLevel}</div>
      <div style="font-size:12px;opacity:.85;"><b>Reasons:</b>
  ...
  // Grab owner and top holders dynamically
  function getCoinData() {
    // Owner from "Created by" section
    const creatorEl = [...document.querySelectorAll("div")].find(el => /Created by/i.test(el.textContent));
    const ownerName = creatorEl?.nextElementSibling?.textContent || "Unknown";
    const ownerPercent = 100; // default; replace if actual % available

    // Top holders (example: find elements with class top-holder)
    const topHolderEls = document.querySelectorAll(".top-holder"); // replace selector
    const topHolders = [...topHolderEls].slice(0,3).map(el=>{
      const name = el.querySelector(".name")?.textContent || "Unknown";
      const percent = parseFloat(el.querySelector(".percent")?.textContent.replace("%","")) || 0;
      return {name, percent};
    });
